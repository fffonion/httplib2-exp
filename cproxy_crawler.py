...
start='http://newcdb.sinaapp.com/index.php/Client/V%s/Connect/?%s'%(cproxyver.replace('.',''),rrange(100,999))
#targ='http://newcdb.sinaapp.com/index.php/Rest/WPConfig'
qstring='u=jiangzemin&p=jzmjzmjzm&v=%s&t=wp&l=wpl%d'
#http://mycdb.sinaapp.com/index.php/Rest/WPTrafficRevc?&tguid=423b7d8881f061c56b26308e1d744d94&ll=1048757
ht=httplib2.Http()
# def get_tguid(wpid):
# 	resp,ct=ht.request(start,method='POST',headers=hdstart,body=qstring%(cproxyver,wpid))
...
while(True):
	...
	# 	tguid=get_tguid(0)
	# 	i=0
	resp,ct=ht.request(start,method='POST',headers=hdtarg,body=qstring%(cproxyver,wpid))
	#print ct
