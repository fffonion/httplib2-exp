...
if url==[]:
    ...
    if not os.path.exists(session_file):
        print('You\'ve connected to Internet, but no session found.')
    else:
        logout_url=open(session_file,'r').read()
        #logout
        resp,ct=ht.request(logout_url,method='GET')
    ...
    resp,ct=ht.request(resp['location'],headers=header)
    #test if successful
    #cryptarg=re.findall('window.location.replace\("\.\/userV2\.do\?method=goToAuthResult&(\mac\=.+\&wlanuserip\=.+&nasip=.+)\&t',ct)
    errmsg=re.findall('errorMessage.innerHTML = \'<strong>(.+)</strong>',ct)
    if errmsg==[]:
        #re.findall('window.location.replace\("\.\/userV2\.do\?method=goToAuthResult&(\mac\=.+\&wlanuserip\=.+&nasip=.+)\&t',ct)
        print('Login succeed!')
        expire=re.findall('id="accountInfo">([^<]+)</span>',ct)[0]#.decode('gb2312')
        print('Expires in: %s'%expire)
        fee=re.findall('accountFeeDisplay" style="display\:block\;">(\d\d\.\d\d)',ct) 
        print('Account fee: '+fee[0])
        #write session args to file for logout
        logout_uri=re.findall('id="tologout" href="\#" localeStr="zh" leaveMode="false" leaveUrl="([^"]+)',ct)[0]
        open(session_file,'w').write('%s%s'%(host_url,logout_uri))
    else:
